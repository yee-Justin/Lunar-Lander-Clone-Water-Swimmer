...
enum EntityType { PLATFORM, PLAYER, ENEMY, BACKGROUND, WALL, FOOD, HEALTH, FIRE, ESCAPE};
...
constexpr float ACC = 0.1f;
constexpr float MAXACC = 50*ACC;
...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[2][10], float animation_time,
    ...
    void normalise_movement() { m_acceleration = MAXACC*glm::normalize(m_acceleration); }

    void face_left() { m_animation_indices = m_walking[0];}
    void face_right() { m_animation_indices = m_walking[RIGHT];}

    void move_left(float ACC) { m_acceleration.x += -ACC; face_left();}
    void move_right(float ACC) { m_acceleration.x += ACC;  face_right(); }
    void move_up(float ACC) { m_acceleration.y += ACC;}
    void move_down(float ACC) {m_acceleration.y += -ACC;}
    ...
    void set_walking(int walking[2][10])
    {
        for (int i = 0; i < 2; ++i)
        {
            for (int j = 0; j < 10; ++j)
